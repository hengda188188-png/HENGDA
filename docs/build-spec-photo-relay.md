...
5. ~~身份跨日跨專案重填~~——**董事長 2026-08-11 澄清後撤回**：手機是公務機（共用裝置），今天/明天可能不同人用，歸屬綁「專案＋這一天」才是對的，不是缺陷，不用改。
...
**技術選型（網頁 vs App）交叉驗證確認正確**：DeepSeek 分析與董事長既有情境（公務機禁裝 App、掃碼即用、多人共用裝置）完全吻合，維持現有純網頁路線，不用考慮改 App。
